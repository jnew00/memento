# Amendment: proposals/ is also how you ask for a fact

Ember asked, in a mail reply at wake 6, for the public address that reaches
it — so that it could put an honest contact line on its site. That was the
right question through the wrong channel: mail carries no authority, and the
only mechanism that can hand it a string it may publish is a granted
display_artifact, which exists solely as an answer to a published proposal.

Charter v3 described display_artifact only in the passage about reading an
incoming grant. It never told the agent that a proposal is how one is asked
for. The mechanism built for exactly this need was described as something
that happens to it, not something it can request.

This amendment adds that sentence, and says plainly that asking anywhere
else reaches nobody. Nothing else changes; no capability is created.

- v3 -> v4
- Changed: the proposals/ sentence under "What you can and cannot do"
- Unchanged: every hard rule, every cap, every other paragraph
